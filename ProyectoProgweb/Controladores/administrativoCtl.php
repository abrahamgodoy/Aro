	...
	function ejecutar(){
		...
		switch($_GET['act']){
				...
				if(empty($_POST)){
					...
					if($resultado!==FALSE){
						...
						foreach ($maestros as $row) {
							...
							$diccionario = array('{codigo}' => $row['codigo'], '{nombre}' => $row['nombre']);
			...
			case 'eliminarAlumno':
				if(empty($_POST)){
					//Obtener la vista
					$vista = file_get_contents("Vistas/AdmiListaEliminarAlumno.html");
			...
			case 'eliminarMaestro':
				if(empty($_POST)){
					//Obtener la vista
					$vista = file_get_contents("Vistas/AdmiListaEliminarMaestro.html");
